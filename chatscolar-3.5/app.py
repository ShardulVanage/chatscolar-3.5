...
load_dotenv("D:/internship/new_flask_app/New/New/api_key.env")
...
def _grade_essay(essay):
    messages = [
        {"role": "system",
        "content": rubric_text+ "You are supposed to grade the English essay and translate your response to Chinese"}
    ]
    essay = "ESSAY : " + essay
...
def chat():
    ...
    if request.method == 'POST':
        ...
        if user_question.lower() in start_greeting:
            print("Predefined msg")
            chat_history.append(HumanMessage(user_question))
            chat_history.append(AIMessage("Hello!"))
            print(chat_history)
